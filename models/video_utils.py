...
from models.pose_utils import get_tensor_from_camera
...
def render(
    ...
    if not opt_cam:
        ...
        print("Opt. pose while metric test...")
        indices = vis_indices if vis_indices is not None else range(len(dataset))
        camera_downscale = trainer._get_downscale_factor()
        for i in tqdm(indices, desc=f"rendering {dataset.split}", dynamic_ncols=True):
            # get image and camera infos
            ...
            models = {key: value for key, value in models.items() if key == 'CamPose'}
            trainer.models = models
            # trainer.models = trainer.models['CamPose']
            ...
            camera_pose = get_tensor_from_camera(cam_infos['camera_to_world']) # .transpose(0, 1)
            ...
            rendering_opt, opa = renderpkg["render"], renderpkg["opa"]
            if gaussians.include_sky:
                sky_color = gaussians.sky_cubemap(view, opa.detach(), camera_pose)
                rendering_opt = rendering_opt + sky_color * (1 - opa)
